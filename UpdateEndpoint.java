...
public class UpdateEndpoint  {
   ...
   @Autowired
    public UpdateEndpoint(HalkomManager telimManager) throws JDOMException {                   
   ...
   @ResponsePayload
    public Element handlePingDbRequest(@RequestPayload Element soapRequest) throws ResponseErrorException, Exception {           
      ...
      //Ovde radimo LOG requesta
        ...
        System.out.println("---- datumvalute:"+contact.getDatumvalute());
        System.out.println("---- datumvalute:"+contact.getDatumprometa());
        
       
        
        try {
            telimManager.update(contact); 
    ...
    protected NalogRequest bindToObject(Element soapRequest) throws Exception {
    	...
    	String idNa = idNAExpression.evaluateFirst(soapRequest).getText();
    	...
    	NalogRequest result = new NalogRequest(Integer.valueOf(idNa),Integer.valueOf(tipdok),idDok,idPrejem,datumvalute,racunb,modb,sklicb,iznos,
    ...
    protected void validateRequest(NalogRequest nalogRequest) throws ValidationFailureException {
    	try {
			...
			Integer id = new Integer(nalogRequest.getIdNA());
    	} catch (Exception e) {
    ...
    protected Element createElement(Nalog contact) throws IOException {
        
        Namespace kbNs = Namespace.getNamespace(NAMESPACE_PREFIX, NAMESPACE_URI);

        Element root = new Element("updateResponse", kbNs);
        
        Element okElement = new Element("isOk", kbNs);
        okElement.setText("OK - update executed");               
                
        
        root.addContent(okElement);
